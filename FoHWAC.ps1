...
function initialize-Addons ($subJson) {
    ...
    if ($json -match "^https{0,1}:{1}/{2}[a-zA-Z_0-9]") {
        ...
        Write-Host -ForegroundColor Yellow "`n`tLoaded remote addons file. Changes will not be saved."
    }
...
function install-Addons ($addonNum) {
    ...
    foreach ($addonObj in $addonsObj.Addons[$start..$end]) {
        ...
        if (($addonVers -eq $configObj.WowVers) -or ($configObj.DownloadOld -eq 1)) {
...
"@
        ...
        switch ($choice) {
            ...
            "n" {add-Addon}
            "c" {
                if ($addonsObj.Addons.count -gt 0) {
                    update-Addon
                } 
                else {
